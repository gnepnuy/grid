...
/**
 * v2要做的事情：
 * 1，把之前固定的交易对（eth/stable）换成自定义，这里要做的就是把之前的一些全局配置放到每个订单里。✅
 * 2，关闭网格的基础退费合为base_token一种
 * 3，swap的时候自动筛选出最佳的池子去交易
 */
contract Sardine is Context, ReentrancyGuard{
    ...
    struct OrderConfig{
        address base_token;
        address quote_token;
        uint256 base_fee;//统一返还baseToken
        uint24 pool_fee;
        uint16 slippage;
    }
    ...
    function createGrid(
        ...
        uint256 _base_fee,
        uint24 _pool_fee,
        uint16 _slippage) external nonReentrant() {


        uint256 current_price = _getPrice(_base_token,_quote_token,_pool_fee);

        require(current_price  <= _max_price 
                && current_price  >= _min_price,'price range is too small');
        require(_share_amount <= _init_amount,'too few grids');

        require(_init_amount > 0,'need to put money in');
        // todo: set investment threshold

        IERC20(_base_token).transferFrom(_msgSender(),address(this),_init_amount);
        OrderConfig memory order_config = OrderConfig(
                                                _base_token,
                                                _quote_token,
                                                _base_fee,
                                                _pool_fee,
        ...
        if(side == Side.Buy){
            _createBuyGrid(order_config,_min_price,_max_price,_init_amount,current_price,_interval_price,_share_amount);
        }else if (side == Side.Sell){
            _createSellGrid(order_config,_min_price,_max_price,_init_amount,_interval_price,_share_amount);
        }else{
            _createBuyAndSellGrid(order_config,_min_price,_max_price,_init_amount,current_price,_interval_price,_share_amount);
        }
    }

    function _createBuyAndSellGrid(
        OrderConfig memory _order_config,
        uint256 _min_price,
        uint256 _max_price,
        uint256 _init_amount,
        uint256 _init_price,
        ...
        _createBuyGrid(_order_config,_min_price,_max_price,_init_amount/2,_init_price,_interval_price,_share_amount);
        _createSellGrid(_order_config,_min_price,_max_price,_init_amount/2,_interval_price,_share_amount);
    }
    ...
    function _createSellGrid(
        ...
        uint256 receive_amount = _swap(
                                    _order_config.base_token,
                                    _order_config.quote_token,
                                    _init_amount,
                                    _order_config.pool_fee,
                                    _order_config.slippage);
        uint256 price = _getPrice(
                            _order_config.base_token,
                            _order_config.quote_token,
                            _order_config.pool_fee);

        uint256 share_amount = (_share_amount * (10 ** IDecimalERC20(_order_config.quote_token).decimals())) / price;
    ...
    function closeOrder(uint256 _order_id)external nonReentrant {
        ...
        if(order.balance > 0){
            if(order.side == Side.Buy){
                IERC20(order.config.base_token).transfer(_msgSender(),order.balance);
            }else{
                IERC20(order.config.quote_token).transfer(_msgSender(),order.balance);
            }
            order.balance = 0;
        }
    ...
    function buy(uint256 _order_id)external nonReentrant{
        ...
        uint256 current_price = _getPrice(
                                    order.config.base_token,
                                    order.config.quote_token,
                                    order.config.pool_fee);
        ...
        uint256 eth_amount = _swap(
                                order.config.base_token,
                                order.config.quote_token,
                                order.share_amount,
                                order.config.pool_fee,
                                order.config.slippage);
        uint256 tx_price = _getPriceByTx(order.share_amount,eth_amount,
                                            IDecimalERC20(order.config.base_token).decimals(),IDecimalERC20(order.config.quote_token).decimals());
    ...
    function closePosition(uint256 _position_id)external nonReentrant{
        ...
        if(position.side == Side.Sell){
            ...
            if(_msgSender() != position.creator){
                IERC20(order.config.quote_token).transfer(_msgSender(),bounty/2);
                IERC20(order.config.quote_token).transfer(position.creator,bounty/2);
            }else {
                IERC20(order.config.quote_token).transfer(_msgSender(),bounty);
            }
            
            order.last_price = _getPriceByTx(position.amount,eth_amount,
                                    IDecimalERC20(order.config.base_token).decimals(),IDecimalERC20(order.config.quote_token).decimals());
            if(order.status == OrderStatus.Close){
                IERC20(order.config.quote_token).transfer(orderToOwner[position.order_id],eth_amount - bounty);
            }else {
            ...
            uint256 bounty = income * 200/1000;
            if(_msgSender() != position.creator){
                IERC20(order.config.base_token).transfer(_msgSender(),bounty/2);
                IERC20(order.config.base_token).transfer(position.creator,bounty/2);
            }else{
                IERC20(order.config.base_token).transfer(_msgSender(),bounty);
            }
            
            order.last_price = _getPriceByTx(stable_amount,position.amount,
                                    IDecimalERC20(order.config.base_token).decimals(),IDecimalERC20(order.config.quote_token).decimals());
            if(order.status == OrderStatus.Close){
                IERC20(order.config.base_token).transfer(orderToOwner[position.order_id],stable_amount - bounty);
            }else {
    ...
    function getPrice(address _base_token,address _quote_token,uint24 _fee) external view returns (uint256 price){
        price = _getPrice(_base_token,_quote_token,_fee);
    }
    ...
    function _getPrice(address _base_token,address _quote_token,uint24 _fee)internal view returns (uint256 price){
        (, int24 tick,,,,,) = IUniswapV3Pool(_getPool(_base_token,_quote_token,_fee)).slot0();
        uint128 ethAmount = 1 ether;
        price = OracleLibrary.getQuoteAtTick(tick,ethAmount,_base_token,_quote_token);
    }
    ...
    function  _swap(
        ...
        uint256 amount_in,
        uint24 fee,
        uint16 slippage)internal returns(uint256 receive_amount){ 

        uint256 token_out_balance_before = IERC20(token_out).balanceOf(address(this));
        ...
        uint256 token_out_balance_after = IERC20(token_out).balanceOf(address(this));
        receive_amount = token_out_balance_after - token_out_balance_before;
    }
